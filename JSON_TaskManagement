...
#Include jsongo.v2.ahk ;download from GroggyOtter's repo https://github.com/GroggyOtter/jsongo_AHKv2

Persistent
#SingleInstance Force

; Define default file name without .json extension
...
; Hotkey to show the GUI = CTRL + ALT + SHIFT + W
^!#w::Main.Show()
...
LoadTasks(*) {
    ...
    LoadPath := FileSelect("Open", DefaultFileName, "Load Task List", "*.json")
    if (!LoadPath) {
        return
    }
...
LoadDefaultTasks(*) {
    ...
    if FileExist(DefaultFileName ".json") {
        LoadFromFile(DefaultFileName ".json")
    }
}
...
LoadFromFile(FilePath) {
    ...
    TaskListView.Delete()
    for Task in Tasks {
        TaskListView.Add("", Task["TaskName"], Task["TaskDescription"], Task["TaskPriority"], Task["TaskDifficulty"], Task["TaskDueDate"], Task["TaskStatus"])
    }
